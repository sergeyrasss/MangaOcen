...
def git_commit_chapters(repo_root, sorted_dir, start_from=None):
    ...
    Формат структуры: sorted_manga/Chapter {номер}/
    """
    os.chdir(repo_root)
    ...
    start_index = 0
    if start_from:
        # Нормализуем формат начальной папки (Chapter X или просто X)
...
if __name__ == "__main__":
    # Конфигурация
    REPO_ROOT = "/home/den/MangaOcen"  # Корень репозитория
    SORTED_DIR = "/home/den/MangaOcen/GrandBlue/sorted_manga"  # Папка с главами
    START_FROM = "Chapter 3"  # Начать с этой главы (None - с первой)
